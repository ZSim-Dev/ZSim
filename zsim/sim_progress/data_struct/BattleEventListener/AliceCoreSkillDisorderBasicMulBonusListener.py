...
class AliceCoreSkillDisorderBasicMulBonusListener(BaseListener):
    ...
    def listening_event(self, event, signal: LBS, **kwargs):
        ...
        if not isinstance(signal, Disorder | PolarityDisorder):
            print(f"【爱丽丝紊乱监听器警告】检测到紊乱触发信号(DISORDER_SPAWN)，但是与之匹配传入的不是紊乱或是极性紊乱类型，而是{type(event)}类型")
            return
        # 当传入的紊乱不是物理属性时直接返回。
    ...
    def listener_active(self, event_obj: Disorder | PolarityDisorder):
        """监听器的激活函数，根据当前紊乱的剩余时间，设定Buff层数"""
        from zsim.sim_progress.Buff.BuffAddStrategy import buff_add_strategy
        rest_tick = event_obj.remaining_tick()
        count = min(rest_tick / 60, 10)     # 最大层数10
        buff_add_strategy(self.buff_index, benifit_list=["enemy"], specified_count=count, sim_instance=self.sim_instance)
        if ALICE_REPORT:
            self.sim_instance.schedule_data.change_process_state()
            print(f"【爱丽丝事件】检测到物理属性的紊乱结算，物理异常的剩余时间为{rest_tick:.1f}tick，使本次紊乱的基础倍率提升 {count * 18:.1f} %！")
